(function(){
 ...
 let floor='', moving=false, catState=0; stage.dataset.floor=''; stage.classList.add('is-closed','is-looking-panel'); root.classList.add('is-awake');
 function hydrateScene(next){
  const scene=root.querySelector('.ae-floor[data-floor="'+next+'"] [data-scene-src]');
  if(scene&&!scene.getAttribute('src')) scene.src=scene.dataset.sceneSrc;
 }
 function setFloor(next){
  if(moving||next===floor)return; moving=true; p3Close.classList.remove('is-open'); stage.classList.remove('is-inside-rf','is-looking-panel'); stage.classList.add('is-moving'); status.textContent='· '+next;
  hydrateScene(next);
  ...
  setTimeout(()=>{stage.classList.remove('is-moving','is-closed');moving=false;},1220);
 }
 buttons.forEach(b=>b.addEventListener('click',()=>setFloor(b.dataset.floor))); buttons.forEach(b=>{b.setAttribute('aria-pressed','false');b.classList.remove('is-active')}); status.textContent='';
 ...
 function openP3(){if(moving||stage.dataset.floor!=='P3')return;stage.classList.add('is-p3-pushing');setTimeout(()=>p3Close.classList.add('is-open'),720);setTimeout(()=>stage.classList.remove('is-p3-pushing'),1450)}
 root.querySelectorAll('.ae-p3-open,.ae-p3-hint').forEach(el=>el.addEventListener('click',openP3)); p3Close.addEventListener('click',()=>{p3Close.classList.remove('is-open');stage.classList.remove('is-p3-pushing')});
})();
